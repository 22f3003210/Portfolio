      ...
      <div className="space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-6">
            <span className="text-xs font-extrabold uppercase tracking-widest text-[#0170B9] bg-[#0170B9]/5 px-3 py-1 border border-[#0170B9]/15 rounded-none inline-block font-mono">
              METHODOLOGY
            </span>
            <h2 className="text-3xl md:text-5xl font-black mt-3 text-[#0B1E2E] leading-tight uppercase tracking-tight">
              What I Do
            </h2>
            <p className="text-sm md:text-base font-extrabold text-[#2d6215] uppercase tracking-wider mt-2 font-mono">
              From Root Cause to Business Impact
            </p>
          </div>
          <div className="lg:col-span-6 lg:pt-6 text-left">
            <p className="text-navy font-black text-base md:text-lg leading-relaxed border-l-2 border-[#2d6215] pl-4 bg-[#2d6215]/5 py-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-stretch">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 pl-3 pr-5 py-3 flex-grow">
                      ...
                      <div className="flex flex-col gap-0.5 text-left flex-grow">
                        ...
                        <span className="text-slate-600 font-semibold text-xs leading-relaxed group-hover:text-white/95 transition-colors duration-300">
                          {step.desc}
                        </span>
                      </div>
                    </div>
          ...
          <div className="lg:col-span-6 flex flex-col lg:grid lg:grid-rows-[auto_1fr] lg:h-full w-full max-w-[500px]">
            <div className="text-left mb-4 lg:mb-0">
              <span className="text-xs font-extrabold uppercase tracking-widest text-[#2d6215] bg-[#2d6215]/10 px-3 py-1 border border-[#2d6215]/20 rounded-none inline-block font-mono">
                THE VALUE LADDER
              </span>
            ...
            <div className="flex flex-col justify-between lg:h-full lg:mt-6 space-y-2 lg:space-y-0">
              {[
                { label: 'Data Collection', sub: 'Capture what matters', color: 'bg-slate-200', text: 'text-slate-800' },
                { label: 'Data Engineering', sub: 'Structure and clean', color: 'bg-slate-300', text: 'text-slate-900' },
                ...
                { label: 'Continuous Improvement', sub: 'Learn and iterate', color: 'bg-[#8bc34a]', text: 'text-[#0B1E2E]' },
              ].map((tier, idx, arr) => (
                <div key={tier.label} className="flex flex-col items-center w-full">
                  <div className={`w-full rounded-xl px-6 py-4 flex items-center justify-between shadow-sm hover:shadow-md transition-all duration-300 ${tier.color}`}>
                    <div className="text-left">
                      ...
                      <p className={`text-xs font-medium ${tier.text} opacity-80`}>{tier.sub}</p>
                    </div>
                    <span className={`text-xl font-black ${tier.text} opacity-70`}>{String(idx + 1).padStart(2, '0')}</span>
                  </div>
                  {idx < arr.length - 1 && <ArrowDown className="w-4 h-4 text-[#2d6215] my-1" />}
                </div>
              ))}
            </div>
          </div>
